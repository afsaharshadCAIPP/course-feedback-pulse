...
# MyMemoryTranslator wants a specific source code, not "auto" — map langdetect's
# codes to something it recognizes. Unmapped codes are passed through as-is.
_MYMEMORY_LANG_MAP = {
...
def _translate_one(text, lang_code, target="en"):
    """Try Google Translate first, then MyMemory as a fallback provider.
    Returns None (never raises) if both providers fail."""
    try:
    ...
    try:
        src = _MYMEMORY_LANG_MAP.get(lang_code, lang_code)
        if len(text) <= _MYMEMORY_CHUNK_LIMIT:
            return MyMemoryTranslator(source=src, target=target).translate(text)
        # Split on sentence boundaries (fallback to hard chunks) and translate piece by piece.
        parts = re.split(r"(?<=[.!?。！？])\s+", text)
        chunks, current = [], ""
        for part in parts:
            if len(current) + len(part) + 1 > _MYMEMORY_CHUNK_LIMIT:
                if current:
...
def translate_to_english(text, lang_code):
    """Best-effort translation, trying Google Translate then MyMemory as a fallback,
    then finally the original text. This function never raises.
    ...
    It has been removed for stability. Google Translate's cloud servers are sometimes
    blocked from free hosting tiers (a known deep_translator/scraping-style issue), so
    MyMemory is tried as a second, independent provider before giving up."""
...
with st.sidebar:
    ...
    st.markdown('<div class="sidebar-divider"></div>', unsafe_allow_html=True)
    align_original_left = st.checkbox("🔤 Align original feedback to left", value=True,
                                       key="align_original_left",
                                       help="Left-align the original (non-English) feedback text — applies to Single Review and CSV Analysis.")

if align_original_left:
    # Also forces left-to-right, left-aligned text inside the CSV Analysis results
    # table, since Streamlit's data grid otherwise lets RTL scripts (Urdu/Arabic)
    # auto-flip to right-aligned, making the "Original" column inconsistent with
    ...
    if analyze_clicked:
        if user_review.strip() == "":
            ...
            with st.spinner("Analyzing sentiment..."):
                ...
                if lang_code != "en":
                    orig_class = "orig-text-left" if align_original_left else ""
                    with st.container(border=True):
                        st.markdown("#### 🌐 English Translation")
                        st.markdown(f"Detected language: **{lang_name}**")
                        st.markdown("**Original Feedback**")
                        st.markdown(f'<div class="{orig_class}">{html_lib.escape(user_review)}</div>',
                                    unsafe_allow_html=True)
                        st.markdown("**English Translation**")
                        st.write(translated)
                    st.markdown("")
