...
CSV_HEAVY_ENGINE_ROW_LIMIT = 50  # above this, CSV Analysis forces Logistic Regression to avoid throttling
...
def load_models():
    ...
    if not model_exists or not vec_exists:
        st.error(
            f"⚠️ Model files not found in the app folder ({os.getcwd()}).\n\n"
            f"- `coursera_tfidf_logistic_model.pkl` found: {model_exists}\n"
            f"- `coursera_tfidf_vectorizer.pkl` found: {vec_exists}\n\n"
            "Make sure both files are committed to the repo root (not in a subfolder) and pushed to GitHub."
        )
    try:
        if model_exists:
        ...
        st.error(f"⚠️ Error loading model files ({type(e).__name__}): {e}")
    return models
...
def load_distilbert():
    if not TRANSFORMERS_AVAILABLE:
        st.warning("⚠️ DistilBERT unavailable: the `torch`/`transformers` packages aren't installed in this environment.")
        return None
    try:
        ...
        st.warning(f"⚠️ DistilBERT couldn't be loaded from Hugging Face Hub ({type(e).__name__}: {e}). Falling back to Logistic Regression.")
        return None
...
def load_nllb():
    ...
    try:
        ...
        st.warning(f"⚠️ NLLB translation model couldn't be loaded ({type(e).__name__}: {e}). Falling back to Google Translate.")
        return None
...
def batch_predict_lr(texts):
    ...
    if not MODEL_READY:
        st.error("⚠️ The Logistic Regression model or vectorizer file wasn't found/loaded. "
                  "Check that `coursera_tfidf_logistic_model.pkl` and `coursera_tfidf_vectorizer.pkl` "
                  "are in the repo root. Showing an uninformative 33/33/33 result until this is fixed.")
        n = len(clean)
    ...
    try:
        ...
        st.error(f"⚠️ Logistic Regression prediction failed ({type(e).__name__}: {e}). "
                  "Showing an uninformative 33/33/33 result until this is fixed.")
        n = len(clean)
        return np.tile([1 / 3, 1 / 3, 1 / 3], (n, 1)), SENT_ORDER
    ...
    if engine == "Logistic Regression" or not TRANSFORMERS_AVAILABLE:
        ...
        if bundle is None:
            ...
            try:
                ...
                st.warning(f"⚠️ DistilBERT inference failed ({type(e).__name__}: {e}). Falling back to Logistic Regression.")
                final = lr_aligned
    ...
    if TRANSFORMERS_AVAILABLE:
        ...
        if bundle is not None:
            try:
                ...
                st.warning(f"⚠️ DistilBERT inference failed ({type(e).__name__}: {e}). Its confidence score will mirror Logistic Regression.")
        ...
        if user_review.strip() == "":
            ...
            with st.spinner("Analyzing sentiment..."):
                ...
                if distil_available:
                    ...
                    st.caption("DistilBERT couldn't be loaded in this session (falling back to Logistic Regression), so its score currently mirrors Logistic Regression; Combined averages the two.")
        ...
        if df is not None and not df.empty:
            ...
            if st.button("🔍 Analyze CSV", type="primary", use_container_width=True):
                with st.spinner("Analyzing reviews..."):
                    ...
                    if selected_model != "Logistic Regression" and len(texts) > CSV_HEAVY_ENGINE_ROW_LIMIT:
                        st.warning(
                            f"⚠️ This CSV has {len(texts)} rows. To avoid overloading the app's free hosting "
                            f"tier (and getting throttled), files over {CSV_HEAVY_ENGINE_ROW_LIMIT} rows always "
                            f"use the lightweight Logistic Regression engine for the overall sentiment. "
                            f"Use Single Review Analysis for DistilBERT/Combined on individual reviews."
                        )
                        effective_model = "Logistic Regression"

                    preds, confs = batch_predict(texts, engine=effective_model)
