...
SAMPLE_CSV_DATASETS = {
    ...
    "Course Feedback by Course Name (30 rows)": "course_feedbacks_by_course_name.csv",
    "Course Feedback with Details (30 rows)": "course_feedbacks_with_details.csv",
    "Multilingual Course Feedback Test (45 rows)": "multilingual_course_feedback_test.csv",
    "Course Feedback with Sentiment Labels (60 rows)": "Book1.csv",
    "Simple Course Feedbacks (30 rows)": "course_feedbacks.csv",
    "Simple Test Reviews (30 rows)": "test.csv",
    "Full Original Dataset (123,240 rows)": "preprocessed_reviews_by_course.csv",
}
...
MODEL_DESCRIPTIONS = {
    ...
    "DistilBERT": ("DistilBERT",
        "Fine-tuned multilingual transformer (Afsah-2027/coursera-multilingual-distilbert), loaded from Hugging Face Hub."),
    "Combined": ("Combined (Recommended)",
        "Uses both Logistic Regression and multilingual DistilBERT. Averages the probability outputs."),
}
...
# =============================================================
    ...
    if sample_file:
        try:
            ...
            st.caption(f"📂 Loaded bundled sample: **{sample_choice}** (Total rows: {len(df)})")
    ...
    if df is not None and not df.empty:
        ...
        st.success(f"Detected review column: **{review_col}**" + (f" | Course column: **{course_col}**" if course_col else ""))
    ...
    if result is not None:
        ...
        st.download_button("⬇️ Download Full Results CSV", df_to_csv_bytes(preview_df[display_cols]),
